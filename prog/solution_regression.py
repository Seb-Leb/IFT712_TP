...
class Regression:
    ...
    def entrainement(self, X, t, using_sklearn=False):
        ...
        phi_x = [self.fonction_base_polynomiale(x) for x in X]
        self.w = [0, 1]
        ...
        if using_sklearn:
            ...
            clf.fit(phi_x,t)
            self.w = np.array([clf.intercept_,  *clf.coef_])
            print(self.w)
    ...
    def prediction(self, x):
        ...
        print(phi_x, self.w.reshape(-1, 1))
        y = np.matmul(phi_x, self.w.reshape(-1, 1))
        return y
